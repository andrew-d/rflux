...
describe("Store", function() {
    ...
    describe('waitFor', function() {
        it('will call through to the underlying dispatcher', function() {
            var s = new Store(),
                m = jest.genMockFunction();
            ...
            expect(m.mock.calls[0][0]).toBe(s);
            expect(m.mock.calls[0][1]).toBe(stores);
            expect(m.mock.calls[0][2]).toBe(cb);
        });
    });
});
